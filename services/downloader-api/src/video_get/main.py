...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    registry = ProviderRegistry([YtDlpProvider()])
    manager = JobManager(
        registry=registry,
    ...
    application = FastAPI(title="Video Get API", version=__version__)
    application.state.settings = resolved
    application.state.repository = repository
    application.state.registry = registry
    application.state.job_manager = manager
    application.state.ffmpeg_available = bool(shutil.which("ffmpeg") and shutil.which("ffprobe"))
